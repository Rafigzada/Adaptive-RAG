...
class AnswerGenerator:
    ...
    @tenacity.retry(
        ...
        wait=tenacity.wait_exponential(multiplier=15, min=1, max=120),
    ...
    def _make_gemini_api_call(self, messages: List[Dict[str, str]],
        ...
        Includes retry logic for transient errors via tenacity decorator.
        
        This method will raise exceptions directly if retries are exhausted or for non-retriable errors.
        """
    ...
    def generate_answer_gemini(self, prompt: str, model: str = None,
        ...
        """
        Generate an answer using the Gemini API via SDK.
        Handles exceptions raised by _make_gemini_api_call after retries.
        """
    ...
    def generate_streaming_answer(self, messages: List[Dict[str, str]],
        ...
        """
        Generate a streaming answer using the Gemini API SDK.
        Yields chunks of the answer as they become available.
        Handles exceptions raised by _make_gemini_api_call after retries.
        """
        actual_model = model if model is not None else self.default_model
        try:
